...
class CumulativeLogisticRegression(GLM):
    ...
    def __init__(self, train_df, priors_df, copy=False, test_df=None):
        super(CumulativeLogisticRegression, self).__init__(train_df, priors_df, copy, test_df)
    ...
    def fit(self, formula, **kwargs):
        ...
        Returns:
            dict of parameter estimates with keys "intercepts", "main", and one key for each grouping factor
        """
        self.initialize(formula, **kwargs)

        # set initial parameters
        self.effects['intercepts'] = kwargs.get('intercepts', np.linspace(-1.0, 1.0, self.J - 1))
    ...
    def _create_response_matrix(self):
        ...
        df = pd.DataFrame({'index': self.train_df.index, 'y': self.train_df[self.response]})
        Y = pd.pivot_table(df, index='index', columns=['y'], aggfunc=len, fill_value=0).as_matrix()
        self.response = Y
    ...
    def _create_hessians(self):
        ...
        for g in self.groupings.keys():
            ...
            H = 0.5 * X.transpose().dot(X) + self.sparse_inv_covs[g].sparse_matrix
    ...
    def _create_output_dict(self):
        ...
        Args:
            None
        Returns:
            dictionary with keys "main", "intercepts", and one key for each grouping factor. \
        ...
        self.results_dict['main'] = pd.DataFrame({'variable': self.main_effects,
                                                  'main_value': self.effects['main']})
        self._create_output_dict_inter()

    def predict(self, new_df):
        """ Use the estimated model to make predictions. \
        New levels of grouping factors are given fixed effects, with zero random effects
