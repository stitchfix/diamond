...
class TestGLM(unittest.TestCase):
    ...
    def test_parse_formula(self):
        ...
        self.assertEqual(self.model.total_num_interactions, self.model.train_df.cyl.nunique())
    ...
    def test_create_penalty_matrix(self):
        ...
        expected_inv_cov_block = np.linalg.inv([[0.123, -1.42], [-1.42, 0.998]])
        actual_inv_cov_block = self.model.sparse_inv_covs["cyl"]._block

        self.assertListEqual(self.model.sparse_inv_covs.keys(), ["main", "cyl"])
    ...
    def test_create_main_design(self):
        ...
        expected_design = [[1, float(row[1].var_a)] for row in self.model.train_df.iterrows()]
        actual_design = self.model._create_main_design()

        self.assertEqual(actual_design.shape, (len(self.model.train_df),
    ...
    def test_create_inter_design(self):
        ...
        expected_design = [[1, float(row[1].var_a), 0, 0] if row[1].cyl == 4 else [0, 0, 1, float(row[1].var_a)]
